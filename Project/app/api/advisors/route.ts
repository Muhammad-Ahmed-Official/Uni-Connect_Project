...
import ContactAdvisor from "@/models/contact_advisor.model"
...
export async function GET() {
    const items = await getOrSetCache<AdvisorEntity[]>(
        ...
        async () => {
            await connectDB();
            const advisors = await ContactAdvisor.find().sort({ name: 1 }).lean();
...
export async function POST(req: NextRequest) {
    ...
    await connectDB();
    const advisor = new ContactAdvisor(parsed.data)
    await advisor.save()
