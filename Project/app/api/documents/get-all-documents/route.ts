import { connectDB } from "@/lib/mongodb";
import departmentModel from "@/models/department.model";
import DocumentSchema from "@/models/document.schema";
import { asyncHandler } from "@/utils/asyncHandler";
import { nextError, nextResponse } from "@/utils/Response";
import { NextRequest, NextResponse } from "next/server";

export const GET = asyncHandler(async (req: NextRequest): Promise<NextResponse> => {
    await connectDB();

    const Department = departmentModel;

    const documents = await DocumentSchema.find()
        .populate("user_id", "-password -department_id -isVerified -role -privacy_settings -notification_preferences")
        .populate("department_id", "departmentName")
        .sort({ createdAt: -1 });

    if (!documents || documents.length === 0) {
        return nextError(404, "No documents found for this department");
    }

    return nextResponse(201, "Documents fetched successfully", documents);
});